...
import getContentVersionId from '@salesforce/apex/WorkspaceDocumentWrapper.getContentVersionId';
...
//      // This assumes that you have the ContentVersionId available
//      this.parameter2 = 'https://raagvitech77-dev-ed.file.force.com/sfc/servlet.shepherd/version/renditionDownload?rendition=SVGZ&versionId=0685i00000BEs6nAAD&page=4';
//      console.log('url========='+this.parameter2);
 
//      // Generate an URL for each page of the PDF document
//      this.pdfPages = [];
//      const numPages = 50; // Replace with the actual number of pages in the PDF document
//      for (let i = 0; i <= numPages; i++) {
//        const pageUrl = this.parameter2 + `&page=${i}`;
//        this.pdfPages.push(pageUrl);
//      }
...
previewHandler(event) {
  this.selectedRecordId = event.target.dataset.id;
  console.log('selectRecordId===========', this.selectedRecordId);

  getContentVersionId({ conDocID: this.selectedRecordId })
    .then(result => {
      console.log('results===>', result);
      this.conVersionId = result;
      console.log('pageCount======>', this.conVersionId);
      this.parameter2 = 'https://raagvitech77-dev-ed.file.force.com/sfc/servlet.shepherd/version/renditionDownload?rendition=SVGZ&versionId=' + this.conVersionId + '&page=';

      // Get the total number of pages from the Apex response or by using an API call
      this.getTotalPages(this.conVersionId)
        .then(numPages => {
          this.pdfPages = []; // Clear the pdfPages array before adding new URLs
          for (let i = 0; i < numPages; i++) {
            const pageUrl = this.parameter2 + i;
            this.pdfPages.push(pageUrl);
          }
          console.log('pagePDF--------------' + this.pdfPages);
          this.showModel = true;
        })
        .catch(error => {
    ...
    .catch(error => {
      console.log('====error in fetchDocuments', JSON.stringify(error));
    });
}

closeHandler() {
  this.showModel = false;
  this.pdfPages = []; // Clear the pdfPages array when closing the modal
}

getTotalPages(conVersionId) {
    // Implement the logic to fetch the total number of pages for the given conVersionId
    console.log('xdfgyh');
    return new Promise((resolve, reject) => {
      // Call the getPDFTotalPages Apex method or your preferred implementation
      getPDFTotalPages({ conVersionId })
        .then(result => {
            console.log('result-------------'+result);
          const totalPages = result.totalPages; // Replace with the actual response attribute or property
          resolve(totalPages);
        })
        .catch(error => {
          reject(error);
    ...
    handelOnClick() {
        ...
        if(correctAnswers <= 1) {
            ...
            // let url = 'https://raagvitech77-dev-ed.lightning.force.com/' + '/sfc/servlet.shepherd/document/download/';
            // console.log('url--------', url);

            // let titleVariable;
            // const docId = this.selectId;
            this.submittedIds.push(this.selectId);
            // Find the document object with the matching ID
